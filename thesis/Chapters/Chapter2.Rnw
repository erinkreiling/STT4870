...
\begin{tikzpicture}%\sffamily
  ...
  \path[ <-, draw, red, thick, name path= path4] (loc1) -- node[above, rotate=63]{Create new project}(fork3);
 ...
 \path[ <->,drkgreen, draw, ultra thick, name path= path6] 
          (auth1) -- node[above, rotate=100]{Push} node[below, rotate=85]{Pull/Fetch} (main3);
    \node[
        name path=mainrepo,
