...
\begin{tikzpicture}%\sffamily
  ...
  \coordinate (fork2) at (4.1,-.48);
  \coordinate (fork3) at (4.5,-1.5);
  ...
  \path[ <-, draw, red, ultra thick, name path= path4] (loc1) -- node[above, rotate=63]{Create new project}(fork3);
 ...
 \path[ <->,drkgreen, draw, ultra thick, name path= path6] 
          (auth1) -- node[above, rotate=82]{Push} node[below, rotate=85]{Pull/Fetch} (main3);
    \node[
        name path=mainrepo,
